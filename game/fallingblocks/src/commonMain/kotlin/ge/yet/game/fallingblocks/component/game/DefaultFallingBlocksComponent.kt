...
import ge.yet.game.fallingblocks.component.game.mapper.stateToModel
...
internal class DefaultFallingBlocksComponent(
    ...
    override val model: Value<FallingBlocksComponent.Model> = store.asValue().map(stateToModel)

    init {
        val labelSubscription = store.labels(observer { label ->
            when (label) {
                is FallingBlocksStore.Label.ToppedOut -> onToppedOut(label.runId)
            }
        })
        scope.launch {
